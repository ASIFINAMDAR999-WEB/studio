           ...
           <div className="w-full flex flex-col gap-3">
                ...
                {plan.priceOptions.map((price) => (
                    <Button asChild key={price} variant="outline" className="w-full text-lg py-6 transition-colors duration-300 hover:bg-primary hover:text-primary-foreground">
                        <Link href={`/payment/options?plan=${encodeURIComponent(`${plan.name} - ${price}`)}`} prefetch={true}>
                            {price}
                        </Link>
                    </Button>
                ))}
            ...
            >
              {isContactAdmin ? (
                ...
                <Link href={`/payment/options?plan=${encodeURIComponent(plan.name)}`} prefetch={true}>{plan.cta}</Link>
              )}
            </Button>
        )}
      </CardFooter>
