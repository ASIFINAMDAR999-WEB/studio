...
export function GlobeAnimation() {
  ...
  useEffect(() => {
    ...
    const initGlobe = () => {
      if (!globeContainerRef.current || typeof THREE === 'undefined' || typeof ThreeGlobe === 'undefined') {
        // If scripts aren't loaded, retry shortly
      ...
      // Animation loop
      const animate = () => {
        if (!globeInstance) return; // Stop animation if globe is cleaned up
    ...
    return () => {
      globeInstance = null; // Cleanup instance
       if (globeContainerRef.current) {
        globeContainerRef.current.innerHTML = '';
      }
    };
